...
- Контейнер запускается от непривилегированного пользователя `clawdbot`
...
| Безопасность | ✅ PASS | Non-root пользователь |
...
3. **Интерактивные команды**: Для интерактивных команд clawdbot (например, `onboard`, `configure`) требуется подключение к контейнеру с TTY:
   ```bash
